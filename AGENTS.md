...
- `src/pages/MorseTrainer.jsx`, `src/components/morse/`, `src/utils/morse.js` and `src/utils/morseTrainer.js` - Morse trainer. The guided course follows LCWO: its exact 41-character order across 40 lessons, timed 1–5 minute random-group copies, configurable character/effective speed, tone, start/group delays, Farnsworth timing and a recommendation to advance after three distinct, completed, unassisted practices at ≥90%, including the newly introduced characters. Lesson selection remains manual. The other panels provide free copying practice, a straight key with sidetone, a visual quiz, and the full reference plus a two-way translator. `morse.js` owns the table, encoding, timing and Web Audio, and is shared with `/q-codes`; `morseTrainer.js` owns the LCWO/Koch order, decks, session generation, grading and quiz building. Keep the timing in `morse.js` and the pedagogy in `morseTrainer.js`.
- `src/utils/morseProgress.js` - versioned `morse_learning_v2` storage separates reception, transmission and visual results. Legacy mixed counters remain untouched. Retention advances only on due, unassisted reviews on distinct days; an entire copied group updates each character’s schedule once. Reception review uses recorded substitutions and overdue characters, strictly within the active deck. Aligned edit operations in `gradeCopy` feed both scores and progress: never return to positional comparison or ignore insertion errors. Run `npm run test:morse` (both suites) after edits.
- `src/utils/leitner.js` - weighted selection helpers used by both trainers; the Q-code trainer retains its original progression. `progress` is `{ [key]: { box, correct, wrong, seen } }`, keyed by Q-code in `/q-codes` while Morse uses the separate per-skill model above; `qcodes.js` re-exports it so the page API did not change.
