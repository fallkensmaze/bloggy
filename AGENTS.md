...
- `fdtd-wasm/src/lib.rs`, `src/pages/FdtdSimulator.jsx` and `src/utils/fdtd*.js` - the 2D TEz Yee-grid simulator. Rust/WebAssembly is the production backend; the numerically equivalent JavaScript implementation is a compatibility fallback and supplies Node regression tests. The boundary is CPML with four recursive-convolution memories and a graded complex-frequency-shift profile. Angular power is obtained from DFT phasors of the radial Poynting flux on an interior circle, and the port impedance from DFT voltage/current at the feed. Keep both explicitly described as 2D estimates: `D2D` is a planar cut and `Z/eta0` is normalized per unit extrusion depth, not the 3D antenna's directivity or impedance in ohms.
