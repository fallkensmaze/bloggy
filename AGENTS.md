...
- `/fdtd-simulator` - axisymmetric 3D FDTD dipole simulator powered by Rust/WebAssembly
...
- `fdtd-wasm/src/lib.rs`, `src/pages/FdtdSimulator.jsx`, `src/components/fdtd/FdtdField3D.jsx` and `src/utils/fdtd*.js` - the axisymmetric 3D dipole solver. Rust/WebAssembly is the production backend; the numerically equivalent JavaScript implementation is the compatibility fallback and supplies Node regression tests. It evolves `Er`, `Ez` and `Hphi` on the meridional `(r,z)` grid, uses the cylindrical-axis update at `r=0`, CPML at the outer radial and axial boundaries, and reconstructs the full volume by revolution. This is a true 3D solution only while geometry and excitation remain rotationally symmetric. The broadband feed monitor provides `Zin(f)` in ohms and `S11` relative to 50 ohms; surface current is derived from the circulation of `Hphi`, and its complex distribution supplies the far-field E-plane pattern and 3D directivity. Keep the Rust and JavaScript public methods numerically equivalent and run `npm run test:fdtd` after either changes.
